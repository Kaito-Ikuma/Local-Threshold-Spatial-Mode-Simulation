...
```bash
cd Local-Threshold-Spatial-Mode-Simulation
...
## Poster Result A–D / Spinodal Phase6 boundary response

学会ポスター向けの追加解析は `results/runs/poster_ABCD/` に分離して保存します。既存の Phase0–5 の CSV、checkpoint schema、periodic dynamics は変更しません。

目的は次の4点です。

- Result A: 各有限 `R` の operational pseudospinodal からの matched distance `s=delta-delta_ps(R,Tobs=50)` に対し、`Gamma_0,micro=A_R s^(p_R)` を測定し、effective/matched microscopic exponent `p_R` が large-`R` で `1/2` に近づくかを調べる。
- Result B: micro data 自身から `Gamma(q)=Gamma0+Dq^2` を測り、`xi_dyn,micro=sqrt(D/Gamma0)` を使う有限波数 collapse を調べる。`Gamma(q)-Gamma(0)=-ln K_R(q)` は microscopic model には仮定しない。
- Result C: deterministic Gaussian closure の境界応答 `Delta u=u_forced-u_baseline` を実空間で fit し、Fourier dispersion と独立な `xi_bnd` を測る。
- Result D: `ghost_dirichlet`、`open_fixed_denominator`、診断用 `open_renormalized` を比較し、`x>=2R` の bulk で `Delta u(x)~exp(-x/xi)` に近づくかを調べる。

理論上の長波長関係は

```text
Gamma(q) = Gamma0 + D q^2
xi_dyn = sqrt(D/Gamma0)
tau(q)/tau(0) = 1/[1+(q xi_dyn)^2]
Delta u(x) ~ exp(-x/xi)
```

です。ただし `xi_dyn` は同じ Fourier dispersion から作る内部整合性の長さであり、`xi_bnd` は境界 profile から独立に fit する長さです。Phase6 の profile fit で `xi_dyn` を固定値として使いません。

有限 `R` の microscopic model では true spinodal の存在を仮定しません。`delta_ps` は既存の `P_esc^cum(Tobs=50)=0.10` で定義した **operational microscopic pseudospinodal-like crossover** です。Gaussian spinodal からの距離 `delta` と matched coordinate `s` は CSV で別列として保持します。

### 実行順

1. 既存の fine pseudospinodal scan が `R=6,12,24,48,96` について完了していることを確認する。
2. SQUID で Result A/B の microscopic runs を `R` ごとに投入する。
3. local serial で Phase6 Result C/D を実行する。
4. SQUID 出力を local の同じパスへ転送し、Result A/B の集約・描画を実行する。

SQUID の production primary は `aggregated_exact`、`prepared_metastable`、Philox、stable block IDs、block checkpoint、MPI LPT distribution を既存 Phase5 のまま利用します。1回のPBS jobは1つの `R` を担当します。

```bash
for R in 6 12 24 48 96; do
  qsub -v POSTER_R="$R",POSTER_M=8192 \
    scripts/run_phase5_squid_poster_AB.sh
done
```

`R=24,48` は modes `0..6`、それ以外は Result A 用の mode `0` が既定です。matched offsets は `0.005,0.0075,0.010,0.015,0.020,0.030,0.040`、primary exponent window は事前固定した `0.010<=s<=0.040` です。Result B の primary dispersion/collapse は `qR<=0.35` のみを使います。

精度基準に応じて checkpoint を append-only に増強できます。理論値への一致を理由に M を増やしてはいけません。

```bash
qsub -v POSTER_R=24,POSTER_M=16384 scripts/run_phase5_squid_poster_AB.sh
qsub -v POSTER_R=24,POSTER_M=32768 scripts/run_phase5_squid_poster_AB.sh
qsub -v POSTER_R=24,POSTER_M=65536 scripts/run_phase5_squid_poster_AB.sh
```

小規模な `direct_J` correctness reference は production と分離して保存します。

```bash
qsub -v POSTER_R=24,POSTER_M=64,POSTER_KERNEL=direct_J \
  scripts/run_phase5_squid_poster_AB.sh
```

Phase6 deterministic closure は local serial で実行します。

```bash
scripts/run_spinodal_phase6_boundary_local.sh
```

小規模確認や任意条件は CLI で変更できます。
...
python3 src/spinodal_phase6_boundary.py \
  ...
  --boundary-types ghost_dirichlet,open_fixed_denominator \
  --steady-tol 1e-10 --max-steps 2000 \
  --output-dir results/runs/poster_ABCD/smoke/phase6
```

SQUIDのCSV/checkpointを転送した後、localでA–Dを集約します。

```bash
scripts/run_poster_ABCD_analysis_local.sh
```

主なCSVは次のとおりです。

- `resultA_microscopic_gamma_scaling.csv`, `resultA_pR_summary.csv`
- `resultB_micro_mode_results.csv`, `resultB_micro_dispersion.csv`, `resultB_micro_length.csv`, `resultB_micro_collapse.csv`, `resultB_micro_collapse_summary.csv`
- `resultAB_direct_J_correctness.csv`（小規模 correctness reference が存在する場合）
- `resultC_boundary_profiles.csv`, `resultC_xi_boundary.csv`, `resultC_xi_scaling.csv`
- `resultD_boundary_comparison.csv`, `resultD_boundary_fit_summary.csv`
- `result_ABCD_summary.csv`

poster primary PNG は `01_ResultA_micro_exponent.png`、`02_ResultB_micro_collapse.png`、`03_ResultC_boundary_length.png`、`04_ResultD_boundary_response.png` です。fit-window、boundary layer、`D/kappa_R`、independent-`xi_bnd` collapse などの補助図は `figures/diagnostics/` に保存します。
